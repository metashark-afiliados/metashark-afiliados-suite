...
import { HelpCircle, Loader2, type LucideProps } from "lucide-react";
...
export const DynamicIcon = memo(({ name, className, ...props }: DynamicIconProps) => {
  if (!name || !(name in icons)) {
    clientLogger.warn(`[DynamicIcon] Ícono '${name}' no encontrado en la librería 'lucide-react'.`);
    return fallbackIcon(className);
  }

  const LucideIcon = dynamic(
    async () => {
      const { [name as LucideIconName]: IconComponent } = await import("lucide-react");
      if (!IconComponent) {
          clientLogger.warn(`[DynamicIcon] Componente de ícono para '${name}' no se pudo cargar.`);
          return () => fallbackIcon(className);
      }
      return IconComponent;
    },
    {
      loading: () => <Loader2 className={cn("h-4 w-4 animate-spin", className)} />,
